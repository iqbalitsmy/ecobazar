...
const PageNav = () => {
    const { pageNav, newStyle } = useContext(PageNavContext);
        ...
        >
            <ul className='flex text-white gap-2 parent mx-[5vw] drop-shadow-2xl'>
                <li className='flex items-center gap-2'>
                    <a href="/" className='hover:fill-green-400 fill-white'>
                ...
                {
                    pageNav.map(({ navLink, title }, i) => {
                        return <li key={i} className='flex gap-4 child'>
                            <a className={`drop-shadow-2xl ${i === (pageNav.length - 1) ? "pointer-events-none text-green-400" : ""}`} href={`${navLink}`}>
...
export const MemoPageNav = React.memo(PageNav);
