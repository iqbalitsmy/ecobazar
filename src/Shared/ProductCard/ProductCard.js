...
const Icons = React.memo(({ handleOpen, handleAddWishlistData }) => (
    <>
        <button
            type='button' title='Add to Wishlist'
            className='bg-gray-100 p-1 md:p-2 rounded-full opacity-75 hover:opacity-100 cursor-pointer block'
            onClick={() => handleAddWishlistData()}
        >
            <svg className='fill-slate-700' height={"20px"} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
...
const ProductCard = ({ productDetail }) => {
    ...
    const handleAddToCartData = useCallback((productDetail) => {
        ...
        <div className='max-h-[320px] max-w-[280px] mx-auto border-solid border-gray-200 hover:border-green-600 border-[1px] rounded-md shadow-md hover:shadow-green-300'>
            ...
            >
                <div className={`absolute right-4 top-4 space-y-2 ${isHovered ? `block` : `hidden`}`}>
                    <Icons handleOpen={handleOpen} handleAddWishlistData={handleAddWishlistData} />
